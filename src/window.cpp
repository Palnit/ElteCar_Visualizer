...
MainWindow::MainWindow() : m_window("Elte Car Visualizer", {1024, 720}) {
    m_threaded = new SharedMemory::ThreadedMultiReaderHandler<HUH::Image>("Images", [](void* pointer, int size) {
        // TODO no copy ?
...
int MainWindow::Run() {
    ...
    while (m_window.Loop()) {
        ...
        m_time.Update();
        cameraData.proj = m_camera.GetPerspectiveProjectionMatrix();
        ...
        (*m_mainCommandPool)[frame_index]->SetViewPort(viewport);
        (*m_mainCommandPool)[frame_index]->SetScissor(viewport);
        (*m_mainCommandPool)[frame_index]->SetClearColor(clearColor);
...
void MainWindow::InitializeRenderPass() {
    ...
    m_lidarRenderPass->AddDependency(
        {.DstSubPassIndex = 0,
         .SrcStageMask =
...
void MainWindow::InitializePipeline() {
    ...
    imVertexFactory.AddVertexStream<HUH::Vector3f>(HUH::RHI::VertexFactory::InputRate::Instance);

    lidarVertexFactory.AddVertexStream<&LidarVertex::pos, &LidarVertex::color>();
    lidarVertexFactory.SetPolygonMode(HUH::RHI::VertexFactory::PolygonMode::Fill);
    ...
    m_imPipeline->Init({m_imRenderPass,
                        imVertexFactory,
                        {
                            ...
                            {HUH::RHI::Pipeline::DescriptorTypes::ImageSampler, 6, HUH::RHI::Shader::Stage::Fragment},
                        },
                        false});
...
void MainWindow::InitializeUniformBuffers() {
    for (size_t i = 0; i < 2; ++i) {
        ...
        m_memoryAllocator->Allocate(modelBuffer, HUH::RHI::MemoryAllocator::Device | HUH::RHI::MemoryAllocator::Host);
        m_imUniformModelBuffers.push_back(modelBuffer);
    }
}